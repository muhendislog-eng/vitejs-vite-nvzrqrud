...
const NEIGHBOR_RADIUS = 5; // ±5
const NEIGHBOR_LIMIT = NEIGHBOR_RADIUS * 2 + 1; // 11
...
const PozAramaMotoru: React.FC<PozAramaMotoruProps> = ({ onSelect, category, currentPos }) => {
  ...
  // Neighbor mode (rowid tabanlı, kesin çalışır)
  ...
  /* ---------------- İlk açılışta (isteğe bağlı) kategori varsa listele ---------------- */
  useEffect(() => {
    if (!dbReady) return;
  ...
  /* ---------------- currentPos gelirse otomatik komşu göster ---------------- */
  useEffect(() => {
    if (!dbReady) return;
  ...
  /* ---------------- Input debounce (KRİTİK: neighbor mode + boş input => bozma) ---------------- */
  useEffect(() => {
    if (!dbReady) return;
  ...
  const handleBackToList = useCallback(() => {
    ...
    setSearchTerm('');
    if (category) performSearch('');
    else performSearch(''); // istersen boş bırakabilirsin; burada hızlıca ilk 25'i getiriyoruz
            ...
            {isNeighborMode && (
              <>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-700 bg-white border border-slate-200 px-2 py-1 rounded-lg hover:bg-slate-50 transition-colors"
