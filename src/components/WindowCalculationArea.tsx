  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 w-full">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 relative overflow-hidden w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end relative z-10">
          <div>
        ...
        <table className="w-full text-left text-sm table-fixed min-w-[760px]">
          <thead className="bg-slate-200 text-slate-600 text-xs font-bold uppercase">
            <tr>
              ...
              <th className="px-6 py-3 text-center w-24">Adet</th>
              <th className="px-6 py-3 text-right w-48">
                {windowType === 'pvc' ? 'PVC Profil (kg)' : 'Alüminyum Profil (kg)'}
              </th>
              <th className="px-6 py-3 text-right w-32">İşlem</th>
            </tr>
          </thead>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => {
                ...
                return (
                  <tr
                    key={item.id}
                    className={`hover:bg-slate-50 ${editingId === item.id ? 'bg-orange-50' : ''}`}
                  >
